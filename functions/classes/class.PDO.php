...
abstract class DB {
	...
	public function runQuery($query, $values = array(), &$rowCount = null) {
		...
		return $this->html_escape_data($result);
	}
	 ...
	 * @return mixed
	 */
	private function html_escape_data(&$data) {
		if (is_array($data)) {
			foreach ($data as $i => $v) {
				if (is_array($v) || is_object($v)) {
					$data[$i] = $this->html_escape_data($v);
				}
			}
			return $data;
		}
	...
	public function getObjects($tableName, $sortField = 'id', $sortAsc = true, $numRecords = null, $offset = 0, $class = 'stdClass') {
		...
		return $this->html_escape_data($results);
	}
		...
		if (is_object($statement)) {
			if ($callback) {
				while ($newObj = $statement->fetchObject('stdClass')) {
					if ($callback($this->html_escape_data($newObj))===false) {
						return false;
					}
				}
			}
	...
	public function getObject($tableName, $id = null, $class = 'stdClass') {
		...
		if ($resultObj === false) {
			...
			return $this->html_escape_data($resultObj);
		}
	}
	...
	public function getValueQuery($query = null, $values = array(), $class = 'stdClass') {
		...
		if (is_object($obj)) {
			$obj = (array)$obj;
			return $this->html_escape_data(reset($obj));
		} else {
			return null;
		}
	}
	...
	public function escape_result_fields($result_fields) {
		...
		return $this->html_escape_data($result_fields);
	}
